...
cdef class PorousFlow(_FlowBase):
    ...
    def __cinit__(self, _SolutionBase thermo, *args, **kwargs):
        ...
        self.gas = thermo
    #def setupGrid(self, n, z):
    #    (<CxxPorousFlow*>self.flow).setupGrid(n, z)
    
    # def eval(self, j, rdt, x, r, mask):
    #     (<CxxPorousFlow*>self.flow).eval(j, x, r, mask, rdt)
    
    # def solid(self, x, h_conv, scond, RK, Omega, srho, sCp, rdt):
    #     (<CxxPorousFlow*>self.flow).solid(x, h_conv, scond, RK, Omega, srho, sCp, rdt)
    
    # def restore(self, dom, soln, loglevel):
    #     (<CxxPorousFlow*>self.flow).restore(dom, soln, loglevel)
    ...
    #         cdef np.ndarray[np.double_t, ndim=1] data = \
    #             np.ascontiguousarray(grid, dtype=np.double)
    #         (<CxxPorousFlow*> self.flow).setupGrid(len(data), &data[0])
...
cdef class PorousSim1D(PorousFlow):
    ...
    def eval(self, j, rdt, x, r, mask):
        ...
        :param rdt:
           Reciprocal of the time-step
           size_t, double*, double*, int*, double
        """
        cdef np.ndarray[np.double_t, ndim=1] x_data = \
            np.ascontiguousarray(x, dtype=np.double)

        cdef np.ndarray[np.double_t, ndim=1] r_data  = \
            np.ascontiguousarray(r, dtype=np.double)

        cdef vector[int] mask_data
        for n in mask:
            mask_data.push_back(n)

        (<CxxPorousFlow*>self.flow).eval(j, &x_data[0], &r_data[0], &mask_data[0], rdt)
    ...
    def solve(self, loglevel=1, refine_grid=True, auto=False):
        ...
        flow_domains = [D for D in self.domains if isinstance(D, PorousFlow)]
        zmin = [D.grid[0] for D in flow_domains]
        zmax = [D.grid[-1] for D in flow_domains]
